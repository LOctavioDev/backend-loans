...
@user.get("/users", response_model=List[schemas.user.User], tags=["Users"])
async def read_users(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    db_users = crud.users.get_users(db=db, skip=skip, limit=limit)
    return db_users


@user.get("/user/{id}", response_model=schemas.user.User, tags=["Users"])
...
@user.post("/users", response_model=schemas.user.User, tags=["Users"])
...
@user.put("/user/{id}", response_model=schemas.user.User, tags=["Users"])
...
@user.delete("/user/{id}", response_model=schemas.user.User, tags=["Users"])
